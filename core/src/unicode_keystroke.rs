...
pub enum TisError {
    ...
    #[allow(dead_code)] // P1 ime 事件化（跨线程切换的变体）
...
mod macos_impl {
    ...
    // TODO(P1): TIS 必须在 AppKit 主线程执行（macOS 14+ TSM dispatch_assert_queue_fail
    // SIGTRAP）。重写后主线程在 Swift 宿主 —— 事件化 `ime:switch-ascii` / `ime:restore`
    // 到 Swift 主线程执行后回调 core。P0 降级为不切换（streaming_insert 在 CJK IME 下
    // 可能被拦截，默认开关（off）下无影响）。switch_to_ascii_on_main 保留供 P1 复用。
    pub async fn switch_to_ascii() -> Result<Option<PreviousInputSource>, TisError> {
        Ok(None)
    }

    #[allow(dead_code)] // P1 ime:switch-ascii 事件化接回
    ...
    // TODO(P1): 同 switch_to_ascii —— 事件化到 Swift 主线程。P0 no-op。
    pub async fn restore_input_source(
        _prev: Option<PreviousInputSource>,
    ) -> Result<(), TisError> {
        Ok(())
    }

    #[allow(dead_code)] // P1 ime:restore 事件化接回
