...
use self::core_foundation_sys::base::{CFRelease, CFTypeRef};
...
impl Device {
    fn name(&self) -> Result<String, DeviceNameError> {
        ...
        // kAudioDevicePropertyDeviceNameCFString 按 CF Create Rule 返回
        // （调用者负责 CFRelease）。原版只转 Rust String 从不释放——每次 name()
        // 泄漏一个 CFString（zhunji 实测每次录音 +1~2 个，claude 分析定位）；
        // 且 CFStringGetCStringPtr 失败时二次 GetPropertyData 覆盖引用再丢一个。
        // 重写：转换完成后统一释放一次（null 判空，CFRelease(NULL) 会崩溃）。
        let name = unsafe {
            ...
            let result = if c_string.is_null() {
                ...
                let owned = CStr::from_ptr(buf.as_ptr()).to_str().unwrap().to_owned();
                if !device_name.is_null() {
                    CFRelease(device_name as CFTypeRef);
                }
                return Ok(owned);
            } else {
                let owned = CStr::from_ptr(c_string as *mut _)
                    .to_string_lossy()
                    .into_owned();
                if !device_name.is_null() {
                    CFRelease(device_name as CFTypeRef);
